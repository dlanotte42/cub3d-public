...
/*   Updated: 2021/03/25 00:33:42 by zxcvbinz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

static t_game	*ft_raycast_two(t_game *game, int map[24][24], int lineHeight)
{
	game->raycasting.Addvalues.drawStart = -lineHeight / 2 + game->camera.ris_y / 2;
	if(game->raycasting.Addvalues.drawStart < 0)
		game->raycasting.Addvalues.drawStart = 0;
	game->raycasting.Addvalues.drawEnd = lineHeight / 2 + game->camera.ris_y / 2;
	if(game->raycasting.Addvalues.drawEnd >= game->camera.ris_y)
		game->raycasting.Addvalues.drawEnd = game->camera.ris_y - 1;
	game = ft_raycast_set_block(game, map);
	return (game);
}
...
static t_game	*ft_raycast_one(t_game *game, int map[24][24])
{
	...
	game = ft_raycast_two(game, map, game->raycasting.Addvalues.lineheight);
	return (game);
}
...
t_game			*ft_raycast_zero(t_game *game, int map[24][24])
{
	...
	game->raycasting.rayDirX = game->player.dir_x + game->player.plane_x * game->raycasting.cameraX; 
	game->raycasting.rayDirY = game->player.dir_y + game->player.plane_y * game->raycasting.cameraX;
	...
	if(game->raycasting.rayDirX < 0)
	{
		...
		game->raycasting.sideDistX = (game->player.pos_x - game->raycasting.mapX) * game->raycasting.deltaDistX;
	}
	else
	{
		game->raycasting.stepX = 1;
		game->raycasting.sideDistX = (game->raycasting.mapX + 1.0 - game->player.pos_x) * game->raycasting.deltaDistX;
	}
	if(game->raycasting.rayDirY < 0)
	{
		game->raycasting.stepY = -1;
		game->raycasting.sideDistY = (game->player.pos_y - game->raycasting.mapY) * game->raycasting.deltaDistY;
	}
	else
	{
		game->raycasting.stepY = 1;
		game->raycasting.sideDistY = (game->raycasting.mapY + 1.0 - game->player.pos_y) * game->raycasting.deltaDistY;
	}
